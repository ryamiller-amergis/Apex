...
| F5 | Lean intermediate gate: aggregate focused tests (exact paths, `--selectProjects`) + AC/DoD coverage; type-check only contracts consumed by a later wave |
| F6 | Final gate once: applicable `tsc` + impacted tests per project + data-testid/eslint; completion synopsis; no commit/push |
