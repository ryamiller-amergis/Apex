...
import { appUsers, devSessions, prds } from '../db/schema';
...
export interface MyWorkSummaryDependencies {
  listNativeFeatures(input: { userId: string; project: string }): Promise<NativeSummaryFeature[]>;
  listDevSessions(input: { userId: string; project: string }): Promise<SummaryDevSession[]>;
  listAdoWork(input: { userId: string; project: string; since: string }): Promise<AdoSummaryWorkItem[]>;
}
...
export class MyWorkSummaryService {
  ...
  async getSummary(input: {
    ...
    const [features, sessions] = await Promise.all([
      this.dependencies.listNativeFeatures({ userId: input.userId, project: input.project }),
      this.dependencies.listDevSessions({ userId: input.userId, project: input.project }),
    ]);
    let readyCount = 0;
    let inProgressCount = 0;
    const samples: DurationSample[] = [];
...
/** Production dependencies backed by approved PRDs, self-owned dev sessions, and ADO. */
export function createProductionMyWorkSummaryService(): MyWorkSummaryService {
  return new MyWorkSummaryService({
    async listNativeFeatures({ project }) {
    ...
    async listDevSessions({ userId, project }) {
      const rows = await db
        .select({
          id: devSessions.id,
    ...
    async listAdoWork({ userId, project, since }) {
      const [user] = await db
      ...
      const result = await ado.queryWorkItemsByWiql({
        wiql: [
          ...
          `AND [System.AssignedTo] = '${escapedAssignee}'`,
          "AND [System.WorkItemType] IN ('Feature', 'Product Backlog Item', 'Technical Backlog Item', 'Bug')",
          `AND ([System.State] NOT IN ('Done', 'Closed', 'Removed') OR [System.ChangedDate] >= '${since}')`,
        ].join(' '),
