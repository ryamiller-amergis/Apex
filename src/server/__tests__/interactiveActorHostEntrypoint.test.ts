...
describe('interactive actor host dispatch endpoint', () => {
  ...
  it('parses the Dapr-wrapped JSON request body', () => {
    expect(parseInteractiveDispatchRequest({
      body: JSON.stringify({
        threadId: 'thread-1',
        runId: 'run-1',
  ...
  it('rejects invalid JSON and incomplete dispatch identifiers', () => {
    expect(() => parseInteractiveDispatchRequest({ body: '{' }))
      .toThrow('Interactive dispatch body must be valid JSON');
    expect(() => parseInteractiveDispatchRequest({
      body: JSON.stringify({ threadId: 'thread-1', runId: 'run-1' }),
    })).toThrow('Interactive dispatch requires threadId, runId, and dispatchMessageId');
  });
  ...
  it('acknowledges dispatch before actor failure and logs the rejection', async () => {
    let callback:
      | ((content: DaprInvokerCallbackContent) => Promise<unknown>)
      | undefined;
    const listen = jest.fn(async (
    ...
    const handleTurn = jest.fn().mockRejectedValue(new Error('actor unavailable'));
    jest.spyOn(console, 'log').mockImplementation(() => {});
