...
describe('useApexWorkItems', () => {
  ...
  it('fetches items list for project', async () => {
    const items = [{ id: '1', title: 'Test', status: 'idea', project: PROJECT }];
    mockFetch.mockResolvedValue(ok(items));

    const { result } = renderHook(
      () => useApexWorkItems({ project: PROJECT }),
      { wrapper: makeWrapper() },
    );
  ...
  it('does not fetch when project is null', async () => {
    renderHook(() => useApexWorkItems(null), { wrapper: makeWrapper() });
  ...
  it('passes owner filter in query string', async () => {
    ...
    renderHook(
      () => useApexWorkItems({ project: PROJECT, ownerId: 'user-1' }),
      { wrapper: makeWrapper() },
    );
  ...
  it('passes types filter joined by comma', async () => {
    mockFetch.mockResolvedValue(ok([]));
    renderHook(
      () => useApexWorkItems({ project: PROJECT, types: ['PBI', 'TBI'] }),
      { wrapper: makeWrapper() },
    );
    await waitFor(() => expect(mockFetch).toHaveBeenCalled());
    const url: string = mockFetch.mock.calls[0][0];
...
describe('useMoveApexWorkItem', () => {
  ...
  it('optimistically updates item status in cache', async () => {
    const item = { id: 'item-1', status: 'idea', position: 0, title: 'T', project: PROJECT };
    mockFetch.mockResolvedValue(ok({ ...item, status: 'ready' }));

    const filters = { project: PROJECT };
    const qc = new QueryClient({ defaultOptions: { queries: { retry: false } } });
    const key = ['apex-work-items', PROJECT, 'list', filters];
    qc.setQueryData(key, [item]);

    const Wrapper: React.FC<{ children: React.ReactNode }> = ({ children }) =>
      React.createElement(QueryClientProvider, { client: qc }, children);

    const { result } = renderHook(() => useMoveApexWorkItem(filters), { wrapper: Wrapper });
    await act(async () => {
  ...
  it('rolls back on error', async () => {
    ...
    const qc = new QueryClient({ defaultOptions: { queries: { retry: false } } });
    const key = ['apex-work-items', PROJECT, 'list', filters];
    qc.setQueryData(key, [{ id: 'item-1', status: 'idea', project: PROJECT }]);
