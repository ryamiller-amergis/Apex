...
export function useExportSession() {
  return useMutation<void, Error & { code?: string }, ExportSessionParams>({
    mutationFn: async ({ sessionId, filename, pages }) => {
      ...
      const enqueueResponse = await fetch(`/api/pdf/sessions/${sessionId}/export`, {
        method: 'POST',
        credentials: 'include',
      ...
      const resultUrl = completed.resultUrl ?? `/api/pdf/jobs/${queued.jobId}/result`;
      const resultResponse = await fetch(resultUrl, {
        credentials: 'include',
        headers: apexProjectHeaders(),
      });
      if (!resultResponse.ok) {
        throw new Error(`Export download failed (HTTP ${resultResponse.status})`);
      }
...
async function pollExportJob(statusUrl: string): Promise<PdfConversionJob> {
  ...
  while (Date.now() < deadline) {
    ...
    if (job.status === 'failed') {
      const error = new Error(job.error?.message ?? 'PDF export failed.') as Error & {
        code?: string;
      };
