...
const DesignPlanReviewView: React.FC = () => {
  ...
  // Sync local editable state from the server plan when it changes and there are no unsaved edits.
  useEffect(() => {
    if (plan && !dirty) {
  ...
  return (
    <div className={styles.container}>
      <div className={styles.header}>
        <div>
          <button className={styles.backBtn} onClick={() => navigate(`/backlog/prd/${prdId}`)} type="button">← Back to PRD</button>
          <h2 className={styles.title}>Design Plan</h2>
          <p className={styles.muted}>
            Review and edit the design decisions below, then generate the designs. Editing the plan first keeps
            regeneration cheap.
          </p>
        </div>
      ...
      {isGenerating ? (
        <div className={styles.generating}>
          ...
          <p className={styles.muted}>Generating the design plan…</p>
        </div>
      ) : (
        <>
