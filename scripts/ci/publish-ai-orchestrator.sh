...
if [[ -z "${AI_RUNS_ACR_NAME:-}" ]]; then
  echo "Skipping AI orchestrator publish: AI_RUNS_ACR_NAME is not set."
  exit 0
fi

DOCKERFILE="runners/ai-orchestrator/Dockerfile"
ENTRYPOINT="dist/server/services/aiOrchestrator/entrypoint.js"
...
if [[ -z "${AI_ORCHESTRATOR_CONTAINER_APP_NAME:-}" ]]; then
  echo "Image pushed. Skipping Container App update: AI_ORCHESTRATOR_CONTAINER_APP_NAME not set."
  exit 0
fi

: "${AI_RUNS_RESOURCE_GROUP:?AI_RUNS_RESOURCE_GROUP is required to update the orchestrator Container App}"

if ! az containerapp show \
  --name "$AI_ORCHESTRATOR_CONTAINER_APP_NAME" \
  --resource-group "$AI_RUNS_RESOURCE_GROUP" &>/dev/null; then
  echo "Image pushed. Skipping Container App update: '${AI_ORCHESTRATOR_CONTAINER_APP_NAME}' not found (apply V2 foundation / orchestrator host first)."
  exit 0
fi

echo "Updating Container App ${AI_ORCHESTRATOR_CONTAINER_APP_NAME}..."
az containerapp update \
  --name "$AI_ORCHESTRATOR_CONTAINER_APP_NAME" \
  --resource-group "$AI_RUNS_RESOURCE_GROUP" \
  --image "$IMAGE"

echo "AI orchestrator image published: ${IMAGE}"
