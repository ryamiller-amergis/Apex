-- Replace Phase 2 of the "Move AI agent execution to a bounded worker tier" initiative.
--
-- SUPERSEDES migration 20260805150000 (seed-phase2-interactive-worker-tier). The earlier Phase 2
-- proposed three Features (FEAT-007/008/009) that dispatched interactive turns through a new
-- `ai-runs-interactive` Service Bus lane feeding a warm Container App. Per the accepted ADR
-- 436850c6-6a98-4cb4-b31e-886ef18c7aec, interactive turns must NOT go through Service Bus: they are
-- dispatched in-cluster to warm Dapr virtual actors on Azure Container Apps behind a WebSocket
-- agent gateway, while Service Bus is retained for the background lane only.
--
-- This migration transforms the approved PRD 45f17f84-bdb9-437e-bb6e-9c10ff5a2b37:
--   * removes the old Phase 2 epic, business rules BR-013..019, the phase-4 entry, and the old
--     PRD Phase 2 markdown, and inserts the new single-FEAT-007 epic (TBI-009..012, PBI-007),
--     new BR-013..019, phase 4, and new PRD Phase 2 markdown;
--   * deletes the three old Phase 2 design docs and inserts one new approved design doc
--     (feature_index 6) for FEAT-007;
--   * strips the old ADR Phase 2 addendum and appends the new WebSocket + Dapr/ACA addendum.
--
-- Idempotent (filter-then-append / strip-then-append) and reversible. Safe to run whether or not
-- 20260805150000 was applied.
--
-- Author: Ryan Miller (110b196f-3f0d-4890-969f-5571085039de). Reviewer/approver: Aneesh (1ceaafde-4871-4411-9ef7-5432c7b4e080).

-- Up Migration

-- 1) PRD backlog_json + markdown: drop the old Phase 2, append the new consolidated Phase 2.
UPDATE prds
...
-- 2) Design docs: remove the three old Phase 2 docs; insert the new FEAT-007 design doc.
...
-- 3) ADR: strip any prior Phase 2 addendum (old interactive OR this one) and append the new one.
UPDATE adrs
...
-- Remove the new FEAT-007 design doc.
DELETE FROM design_docs WHERE id = 'b2d3f4a5-1c2e-4f3a-9b4c-7d8e9f0a1027';

-- Strip the new WebSocket ADR addendum.
UPDATE adrs
SET content = regexp_replace(content, E'\\n\\n<!-- PHASE2-WEBSOCKET-APPENDED -->[\\s\\S]*$', ''),
    updated_at = now()
WHERE id = '436850c6-6a98-4cb4-b31e-886ef18c7aec';

-- Remove the new Phase 2 epic/business rules/phase and the new PRD Phase 2 markdown.
UPDATE prds
SET backlog_json = jsonb_set(
      jsonb_set(
        jsonb_set(
